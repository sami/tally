...
class TestWiring(unittest.TestCase):
    def setUp(self):
        ...
        self.ledger.add_listener(
            ThresholdAlertListener(threshold_pence=5000, output=self.output)
        )
    ...
    def test_end_to_end_shared_expense_workflow(self):
        ...
        # 4. Ledger applies it
        self.ledger.execute(
            ApplyExpenseCommand(self.ledger, expense1, strategy1)
        )
        ...
        self.ledger.execute(
            ApplyExpenseCommand(self.ledger, expense2, strategy2)
        )
